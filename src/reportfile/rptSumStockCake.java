...
import reportModel.reportSumStockCakeModel;
 ...
 * @author s386
 */
public class rptSumStockCake extends javax.swing.JDialog {

    /**
     * Creates new form rptSumStockCake
     */
    ...
    String frmName = "rptSumStockCake";
    ThaiUtilities cvth = new ThaiUtilities();
    private String condition = "";

    public rptSumStockCake(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        inittbl();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(rptSumStockCake.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                rptSumStockCake dialog = new rptSumStockCake(new javax.swing.JFrame(), true);
...
private void inittbl() {
        try {
            ...
            TableColumn tc17 = new TableColumn(1, 200);
            tc17.setHeaderValue("ชื่อสาขา");
            TableColumn tc2 = new TableColumn(2, 50);
            tc2.setHeaderValue("Group");
            TableColumn tc3 = new TableColumn(3, 100);
            tc3.setHeaderValue("รหัสสินค้า (PLU)");
            TableColumn tc4 = new TableColumn(4, 200);
            tc4.setHeaderValue("ชื่อสินค้า / รายการ");
            TableColumn tc5 = new TableColumn(5, 100);
            tc5.setHeaderValue("ราคาขาย");
            TableColumn tc6 = new TableColumn(6, 100);
            tc6.setHeaderValue("หน่วย");
            TableColumn tc7 = new TableColumn(7, 100);
            tc7.setHeaderValue("ยอดยกมา");
            TableColumn tc8 = new TableColumn(8, 100);
            tc8.setHeaderValue("รับจากโรงงาน");
            TableColumn tc9 = new TableColumn(9, 100);
            tc9.setHeaderValue("รับโอน(เข้า)");
            TableColumn tc10 = new TableColumn(10, 100);
            tc10.setHeaderValue("โอน(ออก)");
            TableColumn tc11 = new TableColumn(11, 100);
            tc11.setHeaderValue("ของเสีย");
            TableColumn tc12 = new TableColumn(12, 100);
            tc12.setHeaderValue("Charge");
            TableColumn tc13 = new TableColumn(13, 100);
            tc13.setHeaderValue("ยอดขาย");
            TableColumn tc14 = new TableColumn(14, 100);
            tc14.setHeaderValue("ปรับยอด");
            TableColumn tc15 = new TableColumn(15, 100);
            tc15.setHeaderValue("ยอดคงเหลือ");
            TableColumn tc16 = new TableColumn(16, 100);
            tc16.setHeaderValue("มูลค่าการปรับ");
            TableColumn tc18 = new TableColumn(17, 100);
            tc18.setHeaderValue("มูลค่าคงเหลือ");
            ...
            if (dbutility.dbUtility.Char_Set.equalsIgnoreCase("utf-8")) {
                sql = "SELECT s_bran,name,pdesc,punit1,pgroup,d.pcode as pcode,adjeod"
                        + ",sum(bod) as bod,sum(rec) as rec,sum(tfr) as tfr,sum(wid) as wid,sum(ret) as ret,sum(rfn) as rfn,sum(sal) as sal"
                        + ",sum(sal) as sal,sum(adj) as adj,sum(eod) as eod,sum(pret) as pret "
                        + "FROM dstockcake d left join branfile b on d.s_bran = b.code "
                        + "left join product p on d.pcode = p.pcode "
                        + "where "
                        + "d.s_date>= '" + d1 + "' and d.s_date<='" + d2 + "' "
                        + "and p.pgroup >='" + dept1 + "' and p.pgroup <= '" + dept2 + "' "
                        + "and d.pcode >= '" + plu1 + "' and d.pcode <= '" + plu2 + "' "
                        ...
                        + "and p.pstock = 'Y' "
                        + "and ((Bod<>0) or (PBuy<>0) or (Rec<>0) or (TFR<>0) or (WID<>0) or (Sal<>0) or (RET<>0) or (PRET<>0) or (RFN<>0) or (ADJ<>0) or (EOD<>0) ) "
                        + "group by s_bran,pgroup,pcode order by s_bran,pgroup,pcode";
            } else {
                sql = "SELECT s_bran,name,pdesc,punit1,pgroup,d.pcode as pcode,adjeod"
                        + ",sum(bod) as bod,sum(rec) as rec,sum(tfr) as tfr,sum(wid) as wid,sum(ret) as ret,sum(rfn) as rfn,sum(sal) as sal"
                        + ",sum(sal) as sal,sum(adj) as adj,sum(eod) as eod,sum(pret) as pret "
                        ...
                        + "and d.pcode >= '" + cvth.Unicode2ASCII(plu1) + "' and d.pcode <= '" + cvth.Unicode2ASCII(plu2) + "' "
            ...
            reportSumStockCakeModel treemodel = new reportSumStockCakeModel(sql);
