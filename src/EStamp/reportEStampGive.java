...
public class reportEStampGive extends AbstractTreeTableModel {

    private Statement stmt;
    private reportEStampGive_Bean myroot;
    dbUtility db = new dbUtility();
    showMessage msn = new showMessage();
    ThaiUtilities cvth = new ThaiUtilities();

    public reportEStampGive(String sql, String code) throws SQLException {
        db.dbconnect();
        stmt = null;
        ...
        reportEStampGive_Bean subtree = null;
        myroot = new reportEStampGive_Bean();
        ...
        if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
            while (rs.next()) {
                ...
                if (!tempbranold.equals(tempbran)) {
                    if (subtree != null) {
//                        subtree.getChildren().add(new reportEStampGive_Bean(tempbranold, "", "", " ", " ", " "));
                        subtree.getChildren().add(new reportEStampGive_Bean("รวมของสาขา " + tempbranold,
                                "", "", " ", " ", countgroup + ""));
                        myroot.getChildren().add(subtree);
                        subtree.setU_barcode(tempbranold + "-" + tempbnameold);
                    }
                    subtree = new reportEStampGive_Bean();
                    countgroup = 0;
                    i = 1;
                    ...
                    subtree.getChildren().add(new reportEStampGive_Bean(
                    ...
                    subtree.getChildren().add(new reportEStampGive_Bean(barcode, cam, billno, date, time, point));
                }
            ...
            myroot.getChildren().add(new reportEStampGive_Bean("ยอดรวมทั้งสิ้น",
                    "", " ", " ", " ", counttotal + ""
            ));
        } else {
            while (rs.next()) {
...
//                        ));
    ...
    public Object getValueAt(Object node, int column) {
        reportEStampGive_Bean treenode = (reportEStampGive_Bean) node;
        switch (column) {
            case 0:
    ...
    public int getChildCount(Object parent) {
        reportEStampGive_Bean treenode = (reportEStampGive_Bean) parent;
