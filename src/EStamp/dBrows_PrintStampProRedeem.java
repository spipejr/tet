...
public class dBrows_PrintStampProRedeem extends javax.swing.JDialog {
    ...
    public dBrows_PrintStampProRedeem(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        clearForm();
    }
...
//        ProcessPrint();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dBrows_PrintStampProRedeem.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                dBrows_PrintStampProRedeem dialog = new dBrows_PrintStampProRedeem(new javax.swing.JFrame(), true);
    ...
    private void browsStampPro(JTextField txtOutput) {
        try {
            dBrows_StampProRedeem frm = new dBrows_StampProRedeem(null, true);
            frm.setVisible(true);
            if (frm.getStcode().length() > 0) {
                txtOutput.setText(frm.getStcode());
                txtOutput.requestFocus();
            }
        } catch (Exception e) {
            msn.showMessageWarning(e.getMessage());
    ...
    private boolean insertTempReport() {
        ...
        try {
            String sql = "select * from stampredeem sr "
                    ...
                    + " where sr.sr_campaign >='" + strFrom + "' and sr.sr_campaign <='" + strTo + "' "
                    + "and sr.sr_code >='" + strPFrom + "' and sr.sr_code <='" + strPTo + "' "
                    + "and sr.sr_startdate >='" + strDate1 + "' and sr.sr_startdate <='" + strDate2 + "' "
            ...
            if (rs.getRow() != 0) {
//                do {
//                    if (haveFixBranchGroup(rs.getString("sr.sr_campaign"), rs.getString("sr.sr_code"))) {
...
//                } while (rs.next());
    ...
    private boolean haveFixBranchGroup(String scode, String stcode) {
        ...
        try {
            String sql = "select * from stampredeem_linkbgroup where s_code='" + scode + "' and st_code='" + stcode + "' ";
    ...
    private void ProcessPrint() {
        ...
        if (Mode.equals("Sum")) {
            ...
            String rptName = "stampProRedeemReport";
            ...
            if (insertTempReport()) {
                ...
                parm.put("strDate1", strDate1);
                parm.put("strDate2", strDate2);
                String frmName = "dForm_masterProRedeem";
                String rptName = "stampProRedeemReportDetail";
                Report rpt = new Report();
                rpt.viewReport(rptName, parm, frmName);
            }
        }
    ...
    private void insertFixBranchGroup() {
        try {
            String sql = "select * from stampredeem sr "
                    + "left join stampredeem_linkbgroup sg on sr.sr_code = sg.sr_code "
                    + "and sr.sr_campaign = sg.s_code "
                    + "left join branchgroup bg on sg.bgcode = bg.bgroupcode  "
                    + "left join stampcampaign s on sr.sr_campaign = s.s_campaigncode "
                    + "where sr.sr_campaign >='" + strFrom + "' and sr.sr_campaign <='" + strTo + "' "
                    + "and sr.sr_code >='" + strPFrom + "' and sr.sr_code <='" + strPTo + "' "
                    + "and sr.sr_startdate >='" + strDate1 + "' and sr.sr_startdate <='" + strDate2 + "' "
                    + "order by sr.sr_campaign, sr.sr_code, sg.bgcode";
            PreparedStatement pre1 = dbUtility.con.prepareStatement(sql);
            ResultSet rs1 = pre1.executeQuery();
            rs1.first();
            if (rs1.getRow() != 0) {
                String inst = "insert into stamptempreport(s_code, s_name, st_code, st_name, st_startdate, " //5
                        ...
                        + "description, faggroup, fagdesc) " //13
                        + "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
                if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                    do {
                        PreparedStatement preUp = dbUtility.con.prepareStatement(inst);
                        preUp.setString(1, cvth.ASCII2Unicode(rs1.getString("sr.sr_campaign")));
                        preUp.setString(2, cvth.ASCII2Unicode(rs1.getString("s.s_campaignname")));
                        preUp.setString(3, cvth.ASCII2Unicode(rs1.getString("sr.sr_code")));
                        preUp.setString(4, cvth.ASCII2Unicode(rs1.getString("sr.sr_name")));
                        preUp.setString(5, cvth.ASCII2Unicode(rs1.getString("sr.sr_startdate")));
                        preUp.setString(6, cvth.ASCII2Unicode(rs1.getString("sr.sr_enddate")));
                        preUp.setString(7, cvth.ASCII2Unicode(rs1.getString("sr.sr_day")));
                        preUp.setString(8, cvth.ASCII2Unicode(rs1.getString("sr.sr_saletype")));
                        preUp.setString(9, cvth.ASCII2Unicode(rs1.getString("sr.sr_saleitem")));
                        preUp.setString(10, cvth.ASCII2Unicode(rs1.getString("sr.sr_point")));
                        preUp.setString(11, cvth.ASCII2Unicode(rs1.getString("sg.bgcode")) + " " + cvth.ASCII2Unicode(rs1.getString("bg.bgroupname")));
                        preUp.setString(12, cvth.ASCII2Unicode("0-branchgroup"));
                        preUp.setString(13, cvth.ASCII2Unicode("กลุ่มสาขาที่ร่วมรายการ"));
                        preUp.executeUpdate();
                    } while (rs1.next());
                } else {
                    do {
                        PreparedStatement preUp = dbUtility.con.prepareStatement(inst);
                        preUp.setString(1, rs1.getString("sr.sr_campaign"));
                        preUp.setString(2, rs1.getString("s.s_campaignname"));
                        preUp.setString(3, rs1.getString("sr.sr_code"));
                        preUp.setString(4, rs1.getString("sr.sr_name"));
                        preUp.setString(5, rs1.getString("sr.sr_startdate"));
                        preUp.setString(6, rs1.getString("sr.sr_enddate"));
                        preUp.setString(7, rs1.getString("sr.sr_day"));
                        preUp.setString(8, rs1.getString("sr.sr_saletype"));
                        preUp.setString(9, rs1.getString("sr.sr_saleitem"));
                        preUp.setString(10, rs1.getString("sr.sr_point"));
    ...
    private void insertFixBranch() {
        try {
            String sql = "select * from stampredeem sr "
                    + "left join stampredeem_linkbranch sb on sr.sr_code = sb.sr_code and sr.sr_campaign = sb.s_code "
                    + "left join branfile b on sb.bcode = b.code "
                    + "left join stampcampaign s on sr.sr_campaign = s.s_campaigncode "
                    + "where sr.sr_campaign >='" + strFrom + "' and sr.sr_campaign <='" + strTo + "' "
                    + "and sr.sr_code >='" + strPFrom + "' and sr.sr_code <='" + strPTo + "' "
                    + "and sr.sr_startdate >='" + strDate1 + "' and sr.sr_startdate <='" + strDate2 + "' "
                    + "order by sr.sr_campaign, sr.sr_code,sb.bcode";
            PreparedStatement pre1 = dbUtility.con.prepareStatement(sql);
            ResultSet rs1 = pre1.executeQuery();
            rs1.first();
            if (rs1.getRow() != 0) {
    ...
    private void insertFixPLU() {
        try {
            String sql = "select * from stampredeem sr "
                    + "left join stampredeem_linkplu sp on sr.sr_code = sp.sr_code and sr.sr_campaign = sp.s_code "
                    + "left join product p on sp.pcode = p.pcode "
                    + "left join stampcampaign s on sr.sr_campaign = s.s_campaigncode "
                    + "where sr.sr_campaign >='" + strFrom + "' and sr.sr_campaign <='" + strTo + "' "
                    + "and sr.sr_code >='" + strPFrom + "' and sr.sr_code <='" + strPTo + "' "
                    + "and sr.sr_startdate >='" + strDate1 + "' and sr.sr_startdate <='" + strDate2 + "' "
                    + "order by sr.sr_campaign, sr.sr_code, sp.pcode";
            PreparedStatement pre1 = dbUtility.con.prepareStatement(sql);
            ResultSet rs1 = pre1.executeQuery();
            rs1.first();
            if (rs1.getRow() != 0) {
                ...
                if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                    do {
                        ...
                        preUp.setString(9, cvth.ASCII2Unicode(rs1.getString("sr.sr_saleitem")));
                        preUp.setString(10, cvth.ASCII2Unicode(rs1.getString("sr.sr_point")));
                        preUp.setString(11, cvth.ASCII2Unicode(rs1.getString("sp.pcode")) + " " + cvth.ASCII2Unicode(rs1.getString("p.pdesc")));
                    ...
                    do {
                        ...
                        preUp.setString(9, rs1.getString("sr.sr_saleitem"));
                        preUp.setString(10, rs1.getString("sr.sr_point"));
                        preUp.setString(11, rs1.getString("sp.pcode") + " " + rs1.getString("p.pdesc"));
    ...
    private void insertExceptDate() {
        try {
            String sql = "select * from stampredeem sr "
                    + "left join stampredeem_exceptdate sd on sr.sr_code = sd.sr_code and sr.sr_campaign = sd.s_code "
                    + "left join stampcampaign s on sr.sr_campaign = s.s_campaigncode "
                    + "where sr.sr_campaign >='" + strFrom + "' and sr.sr_campaign <='" + strTo + "' "
                    + "and sr.sr_code >='" + strPFrom + "' and sr.sr_code <='" + strPTo + "' "
                    + "and sr.sr_startdate >='" + strDate1 + "' and sr.sr_startdate <='" + strDate2 + "' "
                    + "order by sr.sr_campaign, sr.sr_code, sd.exp_date";
            System.out.println("insertExceptDate " + sql);
            PreparedStatement pre1 = dbUtility.con.prepareStatement(sql);
            ResultSet rs1 = pre1.executeQuery();
            rs1.first();
            if (rs1.getRow() != 0) {
    ...
    private void ProcessPrintBean() {
        ...
        try {
            ...
            ds = new JRBeanCollectionDataSource(EStamp.EStampReport.dataBeanRedeem.getCollectionRedeem(strFrom, strTo, strPFrom, strPTo, strDate1, strDate2));
            parm.put("companyName", lang.getTextCompanyReport());
            parm.put("application", new GetApplicationName().getApplicationName());
            parm.put("user", new GetUser().getUser());

            String frmName = "dForm_masterProRedeem";
            String rptName = "ES_rptRedeem";
