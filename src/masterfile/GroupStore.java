...
import frmlist.ShowBranchOfStore;
 ...
 * @author s386
 */
public class GroupStore extends javax.swing.JFrame {

    /**
     * Creates new form GroupStore
     */
         DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "groupstore";
    ...
    private void initComponents() {
        ...
        setTitle("กำหนดรหัสลักษณะร้านตามกลุ่มห้างสรรพสินค้า");
        ...
        btnShowBranch.setText("แสดงรายการสาขาในแต่ละกลุ่มห้าง ฯ");
        ...
        lblBtypeCode.setText("รหัสห้างสรรพสินค้า");
...
private void checkuserconfig() {
        ...
        if (userrec.getMast9().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast9().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast9().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getStore());
    ...
    private void btnShowBranchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowBranchActionPerformed
        ShowBranchOfStore f = new ShowBranchOfStore(this,true);
        f.setVisible(true);
        // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(GroupStore.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                new GroupStore().setVisible(true);
            }
        });
    }
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสห้างสรรพสินค้าก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().length()>2)
    {
         JOptionPane.showMessageDialog(this, "กำหนดรหัสห้างสรรพสินค้าไม่ถูกต้อง... (ต้องมีขนาดไม่เกิน 2 ตัวอักษร)");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().trim().equals(""))
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสห้างสรรพสินค้าให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update store set name = ? where code = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into store (code,name) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from store where code ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from store where code ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
    ...
    try {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from store where code=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public int chkbranfile(String code) {
        ...
        try {
            if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bstore ='" + cvth.Unicode2ASCII(code) + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
                ...
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bstore ='" + code + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
