...
import frmlist.ShowBranchOfSize;
 ...
 * @author s386
 */
public class Bshopsize extends javax.swing.JFrame {

    /**
     * Creates new form Bshopsize
     */
    DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "bshopsize";
    ...
    private void initComponents() {
        ...
        lblBtypeCode.setText("รหัสลักษณะร้าน");
        ...
        btnShowBranch.setText("แสดงรายการสาขาในแต่ละลักษณะร้าน");
...
private void checkuserconfig() {
        ...
        if (userrec.getMast7().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast7().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast7().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getSize());
    ...
    private void btnShowBranchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowBranchActionPerformed
        ShowBranchOfSize f = new ShowBranchOfSize(this,true);
        f.setVisible(true);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Bshopsize.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสลักษณะร้านก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    ...
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสลักษณะร้านให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update size set name = ? where code = ?";
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from size where code ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from size where code ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
    ...
    try {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from size where code=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public int chkbranfile(String code) {
        ...
        try {
            if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bsize ='" + cvth.Unicode2ASCII(code) + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
                ...
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bsize ='" + code + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
