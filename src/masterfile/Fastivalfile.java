 ...
 * @author s386
 */
public class Fastivalfile extends javax.swing.JDialog {

    /**
     * Creates new form Fastivalfile
     */
    DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "fastivalfile";
    getLanguage lang = new getLanguage();
    ThaiUtilities cvth = new ThaiUtilities();
    public Fastivalfile(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        checkuserconfig();
    ...
    private void initComponents() {
        ...
        lblBtypeCode.setText("รหัสเทศกาล");
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(txtfCode, javax.swing.GroupLayout.PREFERRED_SIZE, 41, javax.swing.GroupLayout.PREFERRED_SIZE))
        ...
        if (tblBtype.getColumnModel().getColumnCount() > 0) {
            ...
            tblBtype.getColumnModel().getColumn(1).setPreferredWidth(650);
        }
        ...
        setSize(new java.awt.Dimension(939, 529));
        setLocationRelativeTo(null);
    }// </editor-fold>//GEN-END:initComponents
private void checkuserconfig() {
        //MenuName = Mast1
        UserLogin userrec = UserLogin.userlogin;
        if (userrec.getMast14().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast14().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast14().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getFastivalfile());
        ...
        String rptName = "rptfastivalfile";
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Fastivalfile.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                Fastivalfile dialog = new Fastivalfile(new javax.swing.JFrame(), true);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสเทศกาลก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().length()>3)
    {
         JOptionPane.showMessageDialog(this, "กำหนดรหัสเทศกาลไม่ถูกต้อง... (ต้องมีขนาดไม่เกิน 3 ตัวอักษร)");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().trim().equals(""))
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสเทศกาลให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void showdataintb()
{
    ...
    try {
        ...
        while(rs.next())
        {
            ...
            {
            tbBranctype.addRow(new Object[]{cvth.ASCII2Unicode(rs.getString("fast_code")),
                                            cvth.ASCII2Unicode(rs.getString("fast_name"))});
            }
            else
            {
            tbBranctype.addRow(new Object[]{rs.getString("fast_code"),
...
public boolean chkdata(String pk)
{
    ...
    try {
        ...
        String sql = "select * from fastivalfile where fast_code ='"+cvth.Unicode2ASCII(pk)+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            chk = true;
        ...
        {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from fastivalfile where fast_code ='"+pk+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            chk = true;
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update fastivalfile set fast_name = ? where fast_code = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into fastivalfile (fast_code,fast_name) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from fastivalfile where fast_code ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(cvth.ASCII2Unicode(rs.getString("fast_name")));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from fastivalfile where fast_code ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(rs.getString("fast_name"));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
    ...
    try {
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from fastivalfile where fast_code=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
