...
import frmlist.ShowBranchOfBusType;
 ...
 * @author s386
 */
public class Businesstype extends javax.swing.JDialog {

    /**
     * Creates new form Businesstype
     */
    DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "businesstype";
    ThaiUtilities cvth = new ThaiUtilities();
    getLanguage lang = new getLanguage();
    public Businesstype(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        checkuserconfig();
    ...
    private void initComponents() {
        ...
        setTitle("กำหนดรหัสประเภทธุรกิจ");
        ...
        lblBtypeCode.setText("รหัสประเภทธรุกิจ");
        ...
        btnShowBranch.setText("แสดงรายการสาขาในแต่ละประเภทธุรกิจ");
...
private void checkuserconfig() {
        ...
        if (userrec.getMast3().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast3().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast3().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getBussType());
        ...
        String rptName = "rptbusinesstype";
    ...
    private void btnShowBranchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowBranchActionPerformed
        ShowBranchOfBusType f = new ShowBranchOfBusType(new NimbusFrame(), true);
        f.setVisible(true); 
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Businesstype.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                Businesstype dialog = new Businesstype(new javax.swing.JFrame(), true);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสประเภทธุรกิจก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().length()>3)
    {
         JOptionPane.showMessageDialog(this, "กำหนดรหัสประเภทธุรกิจไม่ถูกต้อง... (ต้องมีขนาดไม่เกิน 3 ตัวอักษร)");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().trim().equals(""))
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสประเภทธุรกิจให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void showdataintb()
{
    ...
    try {
        ...
        while(rs.next())
        {
            ...
            {
            tbBranctype.addRow(new Object[]{cvth.ASCII2Unicode(rs.getString("buscode")),
                                            cvth.ASCII2Unicode(rs.getString("busname"))});
            }
            else
            {
            tbBranctype.addRow(new Object[]{rs.getString("buscode"),
...
public boolean chkdata(String pk)
{
    ...
    try {
        ...
        {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from businesstype where buscode ='"+pk+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            chk = true;
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update businesstype set busname = ? where buscode = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into businesstype (buscode,busname) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from businesstype where buscode ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(cvth.ASCII2Unicode(rs.getString("busname")));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from businesstype where buscode ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(rs.getString("busname"));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
    ...
    try {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from businesstype where buscode=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public int chkbranfile(String code) {
        ...
        try {
            if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where buscode ='" + cvth.Unicode2ASCII(code) + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
                ...
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where buscode ='" + code + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
