 ...
 * @author s386
 */
public class BankFile extends javax.swing.JFrame {

    /**
     * Creates new form BankFile
     */
     DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "bankfile";
    ...
    private void initComponents() {
        ...
        lblBtypeCode.setText("รหัสธนาคาร");
        ...
        lblDiscription.setText("ชื่อธนาคาร");
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(txtfCode, javax.swing.GroupLayout.PREFERRED_SIZE, 57, javax.swing.GroupLayout.PREFERRED_SIZE))
        ...
        btnDetete.setRequestFocusEnabled(false);
        ...
        db_btnPrint.setRequestFocusEnabled(false);
        ...
        db_btnExel.setRequestFocusEnabled(false);
...
private void checkuserconfig() {
        ...
        if (userrec.getMast16().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast16().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast16().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getBankFile());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(BankFile.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสธนาคารก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    ...
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสธนาคารให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void showdataintb()
{
    ...
    try {
        ...
        while(rs.next())
        {
            ...
            {
            tbBranctype.addRow(new Object[]{cvth.ASCII2Unicode(rs.getString("bcode")),
                                            cvth.ASCII2Unicode(rs.getString("bname"))});
            }
            else
            {
            tbBranctype.addRow(new Object[]{rs.getString("bcode"),
                                            rs.getString("bname")});
            }
        }
    } catch (Exception e) {
...
public boolean validateSave()
{
    boolean chk = true;
        ...
        else if(txtfDiscription.getText().length()>20)
        {
            JOptionPane.showMessageDialog(this, "กำหนดชื่อธนาคารไม่ถูกต้อง... (ต้องมีขนาดไม่เกิน 20 ตัวอักษร)");
            chk = false;
            txtfDiscription.requestFocus();
        }
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update bankfile set bname = ? where bcode = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into bankfile (bcode,bname) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from bankfile where bcode ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(cvth.ASCII2Unicode(rs.getString("bname")));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from bankfile where bcode ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(rs.getString("bname"));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
    ...
    try {
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from bankfile where bcode=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
