...
import frmlist.ShowBranchOfPlane;
 ...
 * @author s386
 */
public class Bplane extends javax.swing.JFrame {

    /**
     * Creates new form Bplane
     */
     DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "bplane";
    ...
    private void initComponents() {
        ...
        setTitle("กำหนดรหัสลักษณะการเช่า/การลงทุน");

        btnShowBranch.setFont(new java.awt.Font("Norasi", 0, 14)); // NOI18N
        btnShowBranch.setText("แสดงรายการสาขาในแต่ละลักษณะการลงทุน/การเช่า");
        ...
        lblBtypeCode.setText("รหัสลักษณะการเช่า");
...
private void checkuserconfig() {
        ...
        if (userrec.getMast8().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast8().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast8().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getPlane());
    ...
    private void btnShowBranchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowBranchActionPerformed
        ShowBranchOfPlane f = new ShowBranchOfPlane(this,true);
        f.setVisible(true);
        // TODO add your handling code here:
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Bplane.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสลักษณะการเช่าก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    ...
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสลักษณะการเช่าให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update plane set name = ? where code = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into plane (code,name) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from plane where code ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from plane where code ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
    ...
    try {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from plane where code=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public int chkbranfile(String code) {
        ...
        try {
            if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bplane ='" + cvth.Unicode2ASCII(code) + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
                ...
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where bplane ='" + code + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
