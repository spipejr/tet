...
import frmlist.ShowBranchOfBrand;
 ...
 * @author s386
 */
public class Brandfile extends javax.swing.JDialog {

    /**
     * Creates new form Brandfile
     */
    DefaultTableModel tbBranctype ;
    dbUtility db = new dbUtility();
    Statement stmt ;
    String frmName = "brandfile";
    ThaiUtilities cvth = new ThaiUtilities();
    getLanguage lang = new getLanguage();
    public Brandfile(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        checkuserconfig();
    ...
    private void initComponents() {
        ...
        lblBtypeCode.setText("รหัสแบรนด์");
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(txtfCode, javax.swing.GroupLayout.PREFERRED_SIZE, 45, javax.swing.GroupLayout.PREFERRED_SIZE))
        ...
        btnShowBranch.setText("แสดงรายการสาขาในแต่ละแบรนด์");
...
private void checkuserconfig() {
        ...
        if (userrec.getMast4().equals("1")) {
            btnSave.setEnabled(false);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(false);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast4().equals("2")) {
            btnSave.setEnabled(true);
            btnDetete.setEnabled(false);
            menuSave.setEnabled(true);
            menuDelete.setEnabled(false);
        }
        if (userrec.getMast4().equals("3")) {
            btnSave.setEnabled(true);
    ...
    private void db_btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_db_btnPrintActionPerformed
        ...
        ds = new JRBeanCollectionDataSource(report.databeanMasterfile.getBrand());
    ...
    private void btnShowBranchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnShowBranchActionPerformed
        ShowBranchOfBrand f = new ShowBranchOfBrand(new NimbusFrame(), true);
        f.setVisible(true);
    ...
    private void txtfCodeFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtfCodeFocusGained

        // TODO add your handling code here:
    }//GEN-LAST:event_txtfCodeFocusGained
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Brandfile.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                Brandfile dialog = new Brandfile(new javax.swing.JFrame(), true);
...
public boolean ChkValidEnter()
{
    ...
    if(txtfCode.getText().equals(""))
    {
        JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสแบรนด์ก่อน");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().length()>3)
    {
         JOptionPane.showMessageDialog(this, "กำหนดรหัสแบรนด์ไม่ถูกต้อง... (ต้องมีขนาดไม่เกิน 3 ตัวอักษร)");
        chk = false;
        txtfCode.requestFocus();
    }
    else if(txtfCode.getText().trim().equals(""))
    {
         JOptionPane.showMessageDialog(this, "กรุณากำหนดรหัสแบรนด์ให้ถูกต้อง");
        chk = false;
        txtfCode.requestFocus();
        txtfCode.setText("");
    }
...
public void showdataintb()
{
    ...
    try {
        ...
        while(rs.next())
        {
            ...
            {
            tbBranctype.addRow(new Object[]{cvth.ASCII2Unicode(rs.getString("brandcode")),
                                            cvth.ASCII2Unicode(rs.getString("brandname"))});
            }
            else
            {
            tbBranctype.addRow(new Object[]{rs.getString("brandcode"),
...
public void btnsave()
{
    if(validateSave())
    {
        if(chkdata(txtfCode.getText()))
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Update brandfile set brandname = ? where brandcode = ?";
        ...
        else
        {
            ...
            {
               stmt = (Statement) db.con.createStatement();
               String sql = "Insert into brandfile (brandcode,brandname) values(?,?)";
               PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
...
public void enter()
{
    ...
    try {
        if(!db.Char_Set.equalsIgnoreCase("utf-8"))
            {
        stmt = (Statement) db.con.createStatement();
        String sql = "select * from brandfile where brandcode ='"+cvth.Unicode2ASCII(txtfCode.getText())+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(cvth.ASCII2Unicode(rs.getString("brandname")));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
        ...
        {
              stmt = (Statement) db.con.createStatement();
        String sql = "select * from brandfile where brandcode ='"+txtfCode.getText()+"'";
        rs = stmt.executeQuery(sql);
        if(rs.next())
        {
            txtfDiscription.setText(rs.getString("brandname"));
            setFocus(true);
            txtfDiscription.requestFocus();
        }
        else {
...
public void delete()
{
    ResultSet rs = null;
    try {
        if(chkbranfile(txtfCode.getText())==0)
            {
        int confirm = JOptionPane.showConfirmDialog(this, "คุณต้องการลบข้อมูลรายการนี้หรือไม่...?", "Confirm", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    {
                    stmt = (Statement) db.con.createStatement();
                    String sql = "Delete from brandfile where brandcode=?";
                    PreparedStatement prm = (PreparedStatement) db.con.prepareStatement(sql);
        ...
        stmt = (Statement) db.con.createStatement();
            ...
            {
                JOptionPane.showMessageDialog(this, "ไม่สามารถลบข้อมูลได้ เนื่องจากมีข้อมูลอยู่ในสาขาจำนวน "+chkbranfile(txtfCode.getText())+" รายการ");
            }
...
public int chkbranfile(String code) {
        ...
        try {
            if (!db.Char_Set.equalsIgnoreCase("utf-8")) {
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where brandcode ='" + cvth.Unicode2ASCII(code) + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
                ...
                stmt = (Statement) db.con.createStatement();
                String sql = "select * from branfile where brandcode ='" + code + "'";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
                    count++;
                }
